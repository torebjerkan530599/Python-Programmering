class Clock:
    ...
    def inc_day(self):
        if self.__day < self.get_daysInMonth(self.__month, self.__year):
            self.__day += 1
        else:
            self.__day = 1
            self.inc_month()
    ...
    def set_clock(self, year, month, day, hour, min, sec):
        ...
        self.__day = day if 0 < day <= self.get_daysInMonth(month,self.__year) else 1
    ...
    def get_daysInMonth(self, month):        
        Clock.months_31 = [1, 3, 5, 7, 8, 10, 12] 
        Clock.months_30 = [4, 6, 9, 11]
        if month in Clock.months_30:
            return 30
        elif month in Clock.months_31:
            return 31
        else:            
            if self.isLeapYear():
                return 29
