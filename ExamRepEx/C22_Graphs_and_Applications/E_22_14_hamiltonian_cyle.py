...
class Graph:
    ...
    # Return None if the graph contains no Hamiltonian cycle 
    def getHamiltonianCycle(self):
        ...
        def hamiltonian_cycle_util(vertex, count):
            if count == n:
                # Check if there is an edge from the last vertex to the first vertex
                last_vertex = path[-1]
                first_vertex = path[0]
                if self.hasEdge(last_vertex, first_vertex):
                    return path[:]  # Found a Hamiltonian cycle
                else:
                    return None

            for neighbor in self.getNeighbors(vertex):
                if not visited[neighbor.v]:
                    visited[neighbor.v] = True
                    path.append(neighbor.v)
                    result = hamiltonian_cycle_util(neighbor.v, count + 1)
                    if result:
                        return result
                    # Backtrack
        ...
        # Try Hamiltonian cycle starting from each vertex
        for start_vertex in range(n):
            visited[start_vertex] = True
            path.append(start_vertex)
            cycle = hamiltonian_cycle_util(start_vertex, 1)
            if cycle:
                return cycle

        return None  # No Hamiltonian cycle found

    # Helper method to check if there is an edge between two vertices
...
if __name__ == "__main__":
    ...
    cycle = g.getHamiltonianCycle()

    # Print the Hamiltonian cycle if found
    if cycle:
        print("Hamiltonian Cycle:", [g.getVertex(vertex) for vertex in cycle])
    else:
